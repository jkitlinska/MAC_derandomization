...
def get_field_value_from_req(req, field_path):
    """
    Safely pobiera wartość z PROBE_REQ-a.
    Obsługuje:
      - "HT_CAP"  -> szuka w req, a potem w req["DATA"]["HT_CAP"]
      - "DATA.HT_CAP"
      - zagnieżdżone typu "DATA_RTS.SUPP" -> domyślnie spróbuje też "DATA.DATA_RTS.SUPP"
    """
    if not field_path:
        return None
...
def extract_mac_to_label(MACs: List[Dict[str, Any]]):
    labels = {}
    for mac in MACs:
        mac_address = mac.get("MAC")
        label = mac.get("LABEL")
        if mac_address and label:
            labels[mac_address] = label
    return labels
